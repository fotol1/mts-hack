...
class Compute_Similarity_Python:
    def __init__(
        ...
        if similarity == "adjusted":
            ...
            raise ValueError(
                "Cosine_Similarity: value for argument 'mode' not recognized."
        ...
        if row_weights is not None:
            ...
            if dataMatrix.shape[0] != len(row_weights):
                raise ValueError(
                    "Cosine_Similarity: provided row_weights and dataMatrix have different number of rows."
                    "Col_weights has {} columns, dataMatrix has {}.".format(
                        len(row_weights), dataMatrix.shape[0]
                    )
                )
    ...
    def applyAdjustedCosine(self):
        ...
        while end_row < self.n_rows:
            ...
            self.dataMatrix.data[
                self.dataMatrix.indptr[start_row] : self.dataMatrix.indptr[end_row]
    ...
    def applyPearsonCorrelation(self):
        ...
        while end_col < self.n_columns:
            ...
            self.dataMatrix.data[
                self.dataMatrix.indptr[start_col] : self.dataMatrix.indptr[end_col]
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.normalize:
                    ...
                    if self.asymmetric_cosine:
                        denominator = (
                            sum_of_squared_to_alpha[columnIndex] * sum_of_squared_to_1_minus_alpha
                            + self.shrink
                            + 1e-6
                        )
                    else:
                        denominator = (
                            sum_of_squared[columnIndex] * sum_of_squared + self.shrink + 1e-6
                        )
                ...
                # Apply the specific denominator for Tanimoto
                    ...
                    denominator = (
                        ...
                        + (sum_of_squared - this_column_weights) * self.tversky_beta
                        + self.shrink
                        + 1e-6
                    )
                    this_column_weights = np.multiply(this_column_weights, 1 / denominator)
                ...
                relevant_items_partition = (-this_column_weights).argpartition(self.TopK - 1)[
                    0 : self.TopK
                ]
        ...
        W_sparse = sps.csr_matrix(
            (values, (rows, cols)), shape=(self.n_columns, self.n_columns), dtype=np.float32,
        )
...
class Compute_Similarity:
    def __init__(self, dataMatrix, use_implementation="density", similarity=None, **args):
        ...
        columns_with_full_features = np.sum(
            np.ediff1d(sps.csc_matrix(dataMatrix).indptr) == dataMatrix.shape[0]
        )
...
def train_ease(interaction_matrix, path_to_save, l2_norm=100):

    # Grahm matrix is X^t X, compute dot product
    similarity = Compute_Similarity(
        interaction_matrix,
        shrink=0,
        topK=interaction_matrix.shape[1],
        normalize=False,
        similarity="cosine",
    )
    ...
    item_popularity = np.ediff1d(interaction_matrix.tocsc().indptr)
    ...
    logger.info("EASE model is ready")
